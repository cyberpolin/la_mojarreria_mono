...
export function registerPolling(
  ...
  const key = `${chatId}-${listenerId}`;

  // console.log(`🛎️  Nuevo polling registrado ChatSession: ${chatId} ListenerID: ${listenerId}---`);
  // console.log(`---  Total amount of polling ---`);
  // console.log(`---  Total amount of listeners ${JSON.stringify(listeners)} ---`);
  // console.log(`---  Total amount of messageQueue ${JSON.stringify(messageQueue)} ---`);
